...
export default async (request: Request, context: Context) => {
  ...
  if (
    normalizedPath === '/ai-rename-variants' ||
    normalizedPath === '/biblio-rename' ||
    normalizedPath === '/figma-ai-rename-variants'
  ) {
  ...
  document.head.appendChild(o)}initApollo();
    ...
    // PostHog Product Analytics (deferred via requestIdleCallback)
  ...
  s(function(){try{
    ...
    posthog.init('phc_xR0jG1Uj4v7DMFULTIhDe6sr8cQTwBVOfPi88b4gbWM',{api_host:'https://us.i.posthog.com',person_profiles:'identified_only'});
  }catch(e){}});
})();</script>`
      : '';
...
<html lang="en">
  ...
  <body>
    ...
    const securityHeaders: Record<string, string> = {
      'Content-Security-Policy-Report-Only': `default-src 'self' https:; script-src 'self' 'nonce-${nonce}' https://static.hotjar.com https://script.hotjar.com https://assets.apollo.io https://*-assets.i.posthog.com; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: https:; font-src 'self' https:; connect-src 'self' https: https://*.hotjar.com wss://*.hotjar.com https://*.apollo.io https://us.i.posthog.com;`,
    };
...
export const config: Config = {
  path: "/*",
  // Minimal excludedPath - the code's shouldBypassSsr handles most cases
  // Only exclude paths that MUST be handled at the routing level for performance
  excludedPath: [
    // Critical: Netlify functions must not be intercepted
    "/.netlify/*",
